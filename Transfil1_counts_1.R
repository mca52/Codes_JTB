...
Parameters <- R6Class(
  ...
  public = list(
    ...
    deathMosq = 15, #death rate of mosquitos
    minvth = 5, #min V/H ratio = 0 
  ...
  public = list (
    ...
    ## Do worm dynamics
      ...
      for (i in seq_along(names)){
        ...
        numberNewWormsF[inds] <- rpois(townHost,.5 * p$lbda * biteRisk[inds] * VtH[nationIndex[i]] * p$psi1 * p$psi2 * p$s2 * biteRate[inds] * larvae[i] * bnRed)+numberNewWormsF[inds]
        numberWormsDiedM[inds]<- rpois(townHost,p$mu*WM[inds])+numberWormsDiedM[inds]
        numberNewWormsM[inds]<- rpois(townHost,.5 * p$lbda * biteRisk[inds] * VtH[nationIndex[i]] * p$psi1 * p$psi2 * p$s2 * biteRate[inds] * larvae[i] * bnRed)+numberNewWormsM[inds]
      ...
      runMDA = function(pop=1,towns=NA,coverage,drug,compliance){
        if(coverage != treatCov | compliance!= p$sysComp ){ #check syst non-compliance probabilities been set for this coverage level already, if not then set them
          ...
          #p$sysEx <<- exclusion
          #sysExProb <<- rbinom(nHost, 1, p$sysEx)
          treatprob[which(sysExProb==0 & age>=5)] <<- rbeta(length(which(sysExProb==0 & age>=6)),shape1=treatCov*(1-p$sysComp)/p$sysComp,shape2=(1-treatCov)*(1-p$sysComp)/p$sysComp) # for those who are NOT part of that group this is their treatment probability 
          treatprob[which(sysExProb==1 | age<5)] <<- 0 # for those who are part of that group, their treatment probability is 0 - they will never receive treatment 
